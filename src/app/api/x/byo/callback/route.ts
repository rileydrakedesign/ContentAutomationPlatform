...
// GET /api/x/byo/callback - OAuth 1.0a callback for BYO app credentials
export async function GET(request: NextRequest) {
  try {
    ...
    if (!byo?.consumer_key || !byo?.consumer_secret) {
      return NextResponse.redirect(new URL("/settings?error=missing_byo_keys", request.url));
    }
    ...
    await supabase.from("x_oauth_requests").delete().eq("user_id", user.id);

    return NextResponse.redirect(new URL("/settings?success=connected", request.url));
  } catch (error) {
    console.error("BYO X OAuth callback error:", error);
