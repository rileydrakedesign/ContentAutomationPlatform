...
- [ ] **Multi-account support.** Let a user connect and manage multiple X accounts under one login, switching the active account for writing, scoring, publishing, and analytics.
...
- [ ] **Reply-via-new-tab workaround (X API reply restriction).** _Investigate:_ X API may not allow posting replies unless the account is mentioned in the target post — confirm whether this restriction is real. If confirmed, add a fallback that lets users still post a reply by opening X in a new tab from the dashboard with the reply text pre-filled (e.g. an intent/compose URL), so replies can still go out despite the API limitation.

## Cleanup / Pivot alignment

- [x] **Drop features not aligned with the new pivot.** ~~Audit and remove features that don't fit the writing-assistant pivot.~~ Done (2026-07-04 alignment pass): reply finder rebuilt on the live assistant (metered voice-check button removed from the web app; on-ramp reframed as "starting points"); onboarding/tour now end in the editor ("Write your first post"), not on analytics; nav reordered Write-first. Deliberate keeps: `/strategy` and the dashboard root stay (owner decision); voice-check lib/endpoints stay (extension, agency, MCP still use them).
