...
| **Reply in voice** — reply-voice + optional check | `/reply` (generate → [optional voice-check] → Post reply / Voice-check & reply) + extension (generate → optional voice-check → native send); both call `POST /generate-reply` | `POST /drafts/generate` (reply), `POST /voice/check` (optional), `POST /publish/now` (`X_REPLY`) | `generate_reply`, `check_draft` (optional), `publish_reply` |
| **Ship** — publish / schedule (with media) | draft editor (X-accurate counter, image/GIF/video + alt text, link preview, reply audience), `/reply` | `POST /publish/now`, `/publish/schedule`; media via `POST /api/x/media/upload` | `publish_post` / `publish_thread` / `publish_reply`, `schedule_post` |
